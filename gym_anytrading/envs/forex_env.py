...
class ForexEnv(TradingEnv):
    ...
    def __init__(self, df: pd.DataFrame, window_size: int, frame_bound: Tuple[int, int], unit_side: str = 'left',
                 render_mode: Optional[str] = None):
        """Initializes the ForexEnv with the given parameters.

        Args: df (pd.DataFrame): The dataset containing price information. window_size (int): The size of the moving
        window that includes the current price and the previous prices. frame_bound (Tuple[int, int]): The start and
        end indices for the trading frame. unit_side (str, optional): The side of the unit for profit calculation (
        'left' or 'right'). Defaults to 'left'. render_mode (Optional[str], optional): The mode for rendering the
    ...
    def process_data(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        self.df = pd.concat([self.df, ta.adx(self.df.High, self.df.Close, self.df.Low)],
                            axis=1, )  # ADX_14 	DMP_14 	DMN_14
        ...
        self.meaningful_df['SMA_50_above_100'] = (self.meaningful_df['SMA_50'] >= self.meaningful_df['SMA_100']).astype(
            int)
